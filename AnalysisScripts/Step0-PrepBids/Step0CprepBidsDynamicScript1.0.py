...
"""Script to put data in bids format, takes command line arguments, example:
python Step0CPrepBidsDynamicScript1.0.py 
-proj_path "/System/Volumes/Data/misc/data12/sjapee/Sebastian-OrientationImagery/Data/" 
-dataDate "20240103" (this should be the name of the folder in which the .ds folders are contained) 
-S "S01" """
def log_print_statements(print_statements, logfile_path):
    "create log statements files"
...
def setup_and_load(raw_path, logfile_path):
    "Load raw files into mne format"
...
def find_events(raw, log_statements=[]):
    "Find events for epoching, crop trailing zeros, then align with photodiode"
...
def prepMarkers():
    ...
    subs   = ['S01']
    block_conditions = {
        'Dynamic': ['0022_Left', '0022_Right', '0067_Left', '0067_Right', '0112_Left', '0112_Right', 
                        '0157_Left', '0157_Right', '0202_Left', '0202_Right', '0247_Left', '0247_Right',
                        '0292_Left', '0292_Right', '0337_Left', '0337_Right', 'catch']}
    trial_data = []
...
def writeMarkers(event_id, raw, events, stim_duration, currentDset, condition, log_statements):
    "Write annotations into raw files"
...
if __name__ == '__main__':
    ...
    for i in range(1, run_length + 1):
        logfiles.append(logfile_dir + f'{str(subid)}{str(i)}.csv')
    ...
    # Write log
    log_print_statements(log_statements, f'./logdir/{S}_prepbidsDynamic.txt')
