...
for subject in subjects:
    ...
    for run_nr in range(4, 5):
        run_str = f'run-{str(run_nr).zfill(2)}'
        base_name = f'sub-{subjid}_ses-1_task-OrientationImageryDynamic_run-{str(run_nr).zfill(2)}'
        ...
        ica.save(f"{save_path}/sub-{subjid}_ses-1_task-OrientationImageryDynamic_run-{str(run_nr).zfill(2)}_step1b-ica.fif", overwrite=True)
        raw.save(f"{save_path}/sub-{subjid}_ses-1_task-OrientationImageryDynamic_run-{str(run_nr).zfill(2)}_step1b-raw.fif", overwrite=True)
