...
for index, row in rundata.iterrows():
    ...
    code = row['Code']
    breakCheck = row['Break']
    imageStimulus = visual.ImageStim(win, stim_filename, size=(stimHeight, stimWidth), units = 'pix', pos = (0, 20))
    ...
    if ismeg:
        ...
        if iseyetracking:
            eyetracker.send_message(el_tracker,code)
        time1 = Gclock.getTime()
        stim_on = draw_stim(win, imageStimulus, photorect_white, lines)
        timeon_list.append(stim_on) #ask lina
        win.flip()
    time1 = Gclock.getTime() #change this later
    keys_pressed = 0
