...
#WINDOW
win = visual.Window(units="pix", fullscr=isfull, color=(0, 0, 0))
...
def check_responses(response_keys):
    ...
    if pressed:
        if pressed == ['q']:
            ...
            return keys_pressed.append(1)
            #timeList.append(last_flip - trialmat.loc[trial,'stim_on']

            
    
def draw_stim(win, stim, photorect_white, lines, trigger_code = 100):
    print('trigger code is', trigger_code)
    stim.draw()
    photorect_white.draw()
    drawFix(lines)
    if ismeg:
        win.callOnFlip(trigger, port = p_port, code=trigger_code)
    return win.flip()
...
responseScreen = visual.TextStim(win, text=response_text, pos=(10, -350), height=50)
responseScreen.draw()
win.flip()
...
scn_width = 1920 #in px
scn_height = 1080 #in px
monitorwidth = 53.5 # monitor width in cm. measured 53.5 in room 315
monitorheight = 30.5 # monitor height in cm. measured 30.5 in room 315
viewdist = 57
dva = 10 #angle
jitter = 0.5
stimAspectRatio = 0.75
dva_width_fixation = 0.2 #in terms of degrees of visual angle
dva_length_fixation = .5 # in terms of degrees of visual angle
dva_stim_height = 30
...
px_width_fixation = deg_to_pix(dva_width_fixation,viewdist,scn_height,monitorheight) #for fixations
px_length_fixation = deg_to_pix(dva_length_fixation,viewdist,scn_height,monitorheight) #for fixations
...
stimHeight = deg_to_pix(dva,viewdist,scn_height,monitorheight)
#calculate stim width from stim height
stimWidth = int(np.round(stimHeight * stimAspectRatio))
...
for index, row in rundata.iterrows():
    ...
    code = row['code']
    imageStimulus = visual.ImageStim(win, stim_filename, size=(1.5*stimHeight, 1.5*stimWidth), units = 'pix')
    keys_pressed = []
    # Present the image frames
    for i in range(int(imFrames)):
        #imageStimulus.draw()
        #photorect_white.draw()
        last_flip = draw_stim(win, imageStimulus, photorect_white, lines, code)
        check_responses(response_keys)
    for i in range(int(fixationFrames + np.random.randint(0,5,1)[0])):
        last_flip = drawISI(win, lines, photorect_black)
        check_responses(response_keys)
    response_list.append(record_response(keys_pressed))
    if ismeg:
